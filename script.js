(() => {
  ...
  if (menuButton && mainMenu && backdrop) {
  ...
  if (notice && dismissNotice) dismissNotice.addEventListener('click', () => notice.remove());
  ...
  $('#mobileLanguage')?.addEventListener('click', () => $('#languageButton')?.click());
  $('#mobileSearch')?.addEventListener('click', () => { const q = prompt('Search this preview'); if (q) { searchInput.value=q; searchInput.dispatchEvent(new Event('input')); searchForm.dispatchEvent(new Event('submit')); }});
  $('#mobileWatch')?.addEventListener('click', () => showToast('Preview only — watchlist is not connected.'));
  $('#mobileMore')?.addEventListener('click', () => $('#toolsButton')?.click());
  ...
  $('#mobileShare')?.addEventListener('click', shareAction);
